...
function Hero() {
  return (
    <section className='relative min-h-screen flex items-center overflow-hidden bg-black'>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="text-left">
            <FadeIn delay={0}>
              <div className="inline-flex items-center gap-2.5 px-[10px] py-[11px] mb-8 bg-linear-to-r from-primary/10 via-primary/15 to-primary/20 border border-primary/20 rounded-full">
                ...
                {STATS.map((stat, index) => {
                  return (
                    <div key={index} className='text-left border-r boder-white/50 pr-10 md:last:border-r-0'>
                      ...
                      <p className="text-sm text-white leading-sung">
                        {stat.label}
                      </p>
                    </div>
                  )
      ...
      {/* Scroll Indicator */}
      <button onClick={()=> scrollToSection('about')} className='absolute bottom-0 left-1/2 -translate-x-1/2 animate-bounce'>
