...
function Projects() {
    ...
    const totalSlides = Math.max(0, filteredProjects.length - visibleCards + 1);

    //This reset fixes current index if it may have only one card
    useEffect(() => {
    ...
    //         });
    //     }
    // }, [selectedCategory]); //use category, not filteredProjects
    ...
    return (
        <section id="projects" className='relative py-20 bg-black overflow-hidden'>
            <div className="absolute inset-0 overflow-hidden">
                ...
                <div className="absolute top-1/3 right-1/3 w-96 h-96 bg-primary/10 opacity-20 rounded-full blur-3xl"></div>
            </div>
                ...
                <FadeIn delay={0}>
                    <div className="text-center mb-12">
                        ...
                        <p className='text-lg text-white/60 max-w-2xl mx-auto'>
                            Showcasing my best work and achievments.
                        </p>
                    </div>
                </FadeIn>
                {/* Category Filter */}
                ...
                <FadeIn delay={200}>
                    <div className="relative">
                        <div ref={scrollContainerRef} className="overflow-x-auto scroll-smooth snap-x snap-mandatory hide-scrollbar">
                            <div className="flex gap-6 pb-4">
                                {filteredProjects.map((project, index) => {
                                    return (
                                        <div data-card key={project.id} className='w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] shrink-0 snap-start'>
                        ...
                        {filteredProjects.length > visibleCards && (
                            <>
                                <button onClick={prevSlide} disabled={currentIndex === 0} className='flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 lg:-translate-x-4 items-center justify-center w-10 h-10 lg:w-12 lg:h-12 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full hover:bg-white/20 transition-all duration-30 disabled:opacity-50 disabled:cursor-not-allowed z-10' aria-label='Previous projects'>
                                    <ChevronLeft className="w-6 h-6 text-white" />
                                </button>
                                <button onClick={nextSlide} disabled={currentIndex >= filteredProjects.length - 1} className='flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 lg:translate-x-4 items-center justify-center w-10 h-10 lg:w-12 lg:h-12 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full hover:bg-white/20 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed z-10' aria-label='Next projects'>
                        ...
                        {filteredProjects.length > visibleCards && (
                            <div className="flex items-center justify-center gap-2 mt-8">
                                {Array.from({ length: totalSlides }).map((_, index) => {
                                    return (<button key={index} onClick={() => scrollToIndex(index)} className={`transition-all duration-300 rounded-full ${index === currentIndex ? 'bg-primary w-6 h-2' : 'bg-white/30 w-2 h-2 hover:bg-white/50'}`} aria-label={`Go to slide ${index + 1}`}>
                                    </button>)
                                })}
                            </div>
                        )}
