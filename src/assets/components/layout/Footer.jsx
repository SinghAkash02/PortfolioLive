  ...
  return (
    <footer className="relative bg-black overflow-hidden border-t border-white/10">
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-primary/10 opacity-10 rounded-full blur-3xl"></div>
      </div>
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-4 lg:px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            ...
            <div>
              ...
              <ul className="space-y-3">
                {NAV_LINKS.map((link) => {
                  return (
                    <li key={link.id}>
                      <button onClick={() => scrollToSection(link.id)} className='group flex items-center gap-2 text-white/60 hover:text-primary transition-all duration-300'>
          ...
          <FadeIn delay={200}>
            <div>
              ...
              <p className="text-white/60 text-sm mb-6 leading-relaxed">
                Let's connect and create somthing amzaing togather.
              </p>
                ...
                {Object.entries(SOCIAL_LINKS).map(([platform, url]) => {
                  ...
                  return Icon ? (
                    <a key={platform} href={url} target='_blank' rel="noopener noreferrer" className='group relative p-4 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 hover:border-primary/50 hover:scale-110 transition-all duration-300' aria-label={`Connect on ${platform}`}>
                      <Icon className="w-5 h-5 text-white/60 group-hover:text-primary transitions-colors duration-300" />
